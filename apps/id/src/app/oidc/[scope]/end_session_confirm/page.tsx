...
import { dbClientToClient } from "@/lib/oidc/interface";
import "@/lib/oidc/setup";
import { buildSubClaim } from "@noo/oidc-server/idToken";
  ...
  const submitAction = async (data: FormData) => {
    ...
    if (query.sub && client) {
      matchingSession = sessions.find(
        (sess) =>
          buildSubClaim(dbClientToClient(client), sess.userId) == query.sub,
      );
