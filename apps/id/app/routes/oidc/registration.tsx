...
export async function action({ request, params }: ActionFunctionArgs) {
  if (request.method === "POST") {
    return createOidcClient(request, params.tenantId!);
  } else if (request.method === "DELETE") {
    return deleteOidcClient(request, params.tenantId!);
  }
...
export async function deleteOidcClient(request: Request, tenantId: string) {
  const tenant = await getTenant(tenantId);
  if (!tenant) {
    return new Response("Not Found", { status: 404 });
  }
...
async function getTenant(tenantId: string) {
  const raw = humanIdToUuid(tenantId, "org");
  if (!raw) {
    return undefined;
  }

  return await Tenants.find(raw);
}
...
export async function oidcClientRegistration(request: Request, tenant: Tenant) {
  ...
  url.pathname = `/oidc/${tenant.domain}/register`;
  url.searchParams.set("client_id", uuidToHumanId(client.id, "oidc"));
  const registrationUri = url.toString();
