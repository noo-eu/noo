...
export class SignupService {
  ...
  async runStep1() {
    const data = this.trim(step1Schema.parse(this.data));
    this.params = data;
    return this.validateStep1(data);
  }

  async runStep2() {
    const data = this.trim(step2Schema.parse(this.data));
    this.params = data;
    return this.validateStep2(data);
  }

  async runStep3() {
    const data = this.trim(step3Schema.parse(this.data));
    this.params = data;
    return this.validateStep3(data);
  }

  async runStep4(data: Record<string, string>) {
    // Validate the schema
    const parsed = finalSchema.safeParse(data);
    if (!parsed.success) {
      return { form: "invalid_data" };
    }

    let errors = await this.validateStep1(parsed.data);
    if (errors) return errors;

    errors = await this.validateStep2(parsed.data);
    if (errors) return errors;

    errors = await this.validateStep3(parsed.data);
    if (errors) return errors;

    await db.insert(users).values({
      username: parsed.data.username,
      normalizedUsername: parsed.data.username.toLowerCase().replace(/\./g, ""),
      passwordDigest: await this.hashPassword(parsed.data.password),
  ...
  private trim<T extends Record<string, string>>(data: T): T {
